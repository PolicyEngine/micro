...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the UK FRS spine from pinned raw tabs, preserving each "
            "stage's declared seeds and draw order."
        )
    )
    parser.add_argument(
        "--frs-raw-dir",
...
class _SampledGraphRootTransform:
    ...
    def graph_implementation_dependencies(self):
        from microcosm.build import frame_sampling

        return (type(self), _sample_spine_frame, frame_sampling)
...
class PreparedUKSpineExecution:
    """Declared raw-source graph and bindings, prepared without numerical execution."""
    ...
    engine: object
    engine_identity: str
    stochastic_contract: object
    frs_release: object


def parse_uk_spine_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse the maintained raw-source request independently of execution."""
    return _parse_args(argv)


def prepare_uk_spine_execution(args: argparse.Namespace) -> PreparedUKSpineExecution:
    """Bind source paths and lazy transforms without fitting models or writing files."""
    _validate_args(args)
    ...
    engine_identity = hashlib.sha256(
        canonical_json_bytes(_rules_engine_provenance())
    ).hexdigest()
    ...
    register_spine_gate_kernel(
        kernels, spec=spec, engine=engine, engine_identity=engine_identity
    )
    return PreparedUKSpineExecution(
        graph=graph,
        kernels=kernels,
        sources=graph_sources,
        spec=spec,
        stage_names=stage_names,
        stages=tuple(stages_by_name[name] for name in stage_names),
        engine=engine,
        engine_identity=engine_identity,
        stochastic_contract=stochastic_contract,
        frs_release=frs_release,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        prepared = prepare_uk_spine_execution(args)
        spec = prepared.spec
        graph = prepared.graph
        stage_names = prepared.stage_names
        stages_by_name = spec.sources.stage_map()
        ...
        graph_sources = prepared.sources
        ...
        stored_stages = load_spine_stage_artifacts(
            graph_manifest, graph_store, stage_names=stage_names
        )
        ...
        if spine_battery is not None:
            materialize_spine_gate_reports(
                graph_manifest,
                graph_store,
                battery=spine_battery,
                gates=spine_gate_manifest,
            )
        if spine_battery is not None:
        ...
        replay_metadata = stored_stages["hmrc_spi_income_spine"]["checkpoint_metadata"]
        if (
            not isinstance(replay_metadata, dict)
            or "replay_payload" not in replay_metadata
        ):
            raise RuntimeError("HMRC SPI spine stage did not record replay evidence.")
        atomic_write_json(replay_sidecar_path, replay_metadata["replay_payload"])
        ...
        fit_weight_records = stored_evidence["fit_weight_records"]
