...
cannot silently change that denominator. This does not turn the declared window
into a full calendar/fiscal year or align the model's annual observation date.
"""
...
def uc_source_month_metadata(months: object) -> dict[str, str]:
    ...
    if months != sorted(set(months)) or len({month[:4] for month in months}) != 1:
        raise ValueError(
...
def validate_uc_source_month_coverage(
    ...
    if encoded is None:
        return registry
    if (
        reference.family != "dwp_universal_credit"
        or reference.value_operation != "calendar_year_average"
    ):
        raise ValueError(
            "UK UC source-month coverage requires a UC calendar_year_average reference."
        )
    try:
    ...
    uc_source_month_metadata(expected)
    ...
    for spec in registry.specs:
        ...
        if resolved != expected:
            raise ValueError(
                f"UK UC source-month coverage for {reference.name!r}: "
                f"expected exactly one fact for each of {expected!r}; "
            ...
            replace(
                ...
                metadata={
                    **dict(spec.metadata),
                    ...
                    "uk_uc_source_month_count": str(len(resolved)),
                    "uk_uc_source_period_basis": "mean_of_declared_months",
                },
            )
        )
