...
class _PopulationKernel(KernelBase):
    def implementation_hash(self) -> str:
        ...
        return source_hash(
            type(self), geography_ladder, national_sampling, rowwise_dataset, graph_evidence
        )
...
def append_uk_population_nodes(
    ...
    *,
    ...
    if any(node.id == "spine.gates.transferred" for node in graph.nodes):
        ...
        spine_gate_inputs = (ArtifactInput("spine_gate", gate.id, "gate_report", SPINE_GATE_REPORT_TYPE),)
        spine_gate_params = {"spine_gate_phase": "transferred", "spine_gate_release_candidate": bool(gate.params["release_candidate"])}
    ...
    nodes.append(
        Node(
            ...
            structural=StructuralDelta.FILTER,
            mass="free",
            params={**common, "fraction": float(sample_fraction), "seed": sample_seed, **spine_gate_params},
