...
SOI_AMOUNT_MEASURE_VARIABLES: dict[str, str] = {
    ...
    # Build H (microcosm#299): SOI Table 1.4 income/loss lines that identify the
    ...
    # Build H (microcosm#299): SOI Table 2.1 "Home mortgage interest" itemized-
    ...
    #
    # microcosm#511: the SOI line is the amount DEDUCTED on Schedule A, i.e.
...
SOI_RETURN_MEASURE_VARIABLES: dict[str, str] = {
    ...
    # Build H (microcosm#299): return counts paired with the Table 1.4 income/
    ...
    # Build H (microcosm#299): return count paired with the Table 2.1 home
    # mortgage interest amount above (indicator sum over itemizers claiming a
    # home mortgage interest deduction). microcosm#511: indicator over the same
...
SOI_VARIABLE_MAP: dict[str, str] = {
    ...
    # Build H (microcosm#299): concept -> signed model column for the export
    ...
    # microcosm#511 (supersedes the Build H gross mapping): SOI Table 2.1 home
...
US_FISCAL_TARGET_SUPPORT_EXCLUSIONS: dict[str, str] = {
    "irs_soi.ty2023.table_1_4.all.other_income_net_loss_amount": (
        ...
        "1040 line 21 in ty2015; Schedule 1 in later years) — microcosm#393's "
        ...
        "(microcosm#507) measured the consequence: the solve must crush truthful "
        ...
        "excluded until the microcosm#393 remedy lands in full: remap E01200 to "
    ...
    "irs_soi.ty2023.table_1_4.all.other_income_net_loss_returns": (
        ...
        "until the microcosm#393 remedy lands in full (remap E01200 to "
    ...
    *,
    ...
    Args:
        ...
        age_targets: Opt into compile-time period aging of dollar-amount
            ...
            (PolicyEngine/microcosm#116, #212). When enabled, dollar amounts
            ...
            counts stay raw. See
            :mod:`microcosm.build.us_runtime.target_aging`.
        allow_unaged_dollar_targets: The period contract is always enforced
            ...
            :class:`~microcosm.build.us_runtime.target_aging.PeriodContractError`
    ...
    if age_targets:
        ...
        # (PolicyEngine/microcosm#116, #212; fact/computed boundary in
...
def _uprate_cross_period_eitc_decompositions(
    ...
    than all-return totals. When Microcosm uses a prior-year decomposition for a
...
def _rebase_stale_soi_taxable_interest_distributions(
    ...
    *,
    ...
    The microcosm#489 adjudication, with every value from the official IRS
    ...
    period; target aging owns the remaining links (microcosm#488).
    """
    ...
    for spec in registry.specs:
        ...
        metadata = {
            **dict(spec.metadata),
            ...
            # there (microcosm#488 chain-completion law).
            "uprating_to_period": control.source_period,
            "uprating_index_source_period": control.source_period,
            "uprating_index_source_record_id": control.source_record_id,
            "uprating_factor": _format_float(factor),
...
def _soi_taxable_interest_control_key_from_fact(
    ...
    # microcosm#489); they never anchor the interest family.
...
def _dynamic_target_key(fact: object) -> tuple[str, ...]:
    ...
    Ledger should retain old source years. A Microcosm build should activate only
...
def _reference_from_ledger_fact(
    ...
    *,
    ...
    if source_name == "jct":
        ...
        # the anchors (microcosm#451 items 3-4; sign and 45B-scope conversion
...
def _soi_reference_from_fact(
    ...
    *,
    ...
    # route, and are identified by their layout alone (microcosm#451 item 3).
...
def _is_soi_total_uprated_decomposition_fact(
    ...
    # hard target, the exact microcosm#489 disease — so it is not rescued and
...
#
# Role-separation guard (microcosm#508 owner adjudication, 2026-07-23): the
    ...
    *,
    ...
    - ``ssi_recipients.by_area_category`` — SSI recipients (a count) in the
      *federally administered* universe, which includes ~115k people receiving
      ...
      witnessed reference per the microcosm#508 owner adjudication (2026-07-23):
    ...
    if _SSA_SSI_BY_AGE_RECORD_SET_TOKEN in record_set_id:
        ...
        # age group (microcosm#470). The age-band rows bind as national
        ...
        # enforces, per the microcosm#508 adjudication. The all-ages row never
        ...
        if lower == "-inf" and upper == "inf":
            return None
        # Role-separation guard, second layer (microcosm#508 sol review
        ...
        if groupby_value_id not in expected_bounds_by_band:
            raise ValueError(
                ...
                "(under_18/age_18_to_64/age_65_or_older, microcosm#470) "
        ...
        if not all(expected_bounds_by_band[groupby_value_id]):
            raise ValueError(
                ...
                "row under the band role (microcosm#508 role separation)."
            )
    ...
    if geography_level == "state":
        ...
        # per the microcosm#508 owner adjudication (2026-07-23) it must not
...
# Microcosm. Release builds must call compile_us_fiscal_target_registry() with an
