...
def _obbba_config_path() -> Path:
    return Path(str(files("microcosm.build.us").joinpath("obbba_reforms.json")))
...
def _tax_expenditure_config_path() -> Path:
    return Path(
        str(files("microcosm.build.us").joinpath("tax_expenditure_reforms.json"))
    )
...
def _state_program_reforms_config_path() -> Path:
    return Path(str(files("microcosm.build.us").joinpath("state_program_reforms.json")))
...
def _state_reforms_config_path() -> Path:
    return Path(str(files("microcosm.build.us").joinpath("state_reforms.json")))
...
class BaselineLevelSpec:
    ...
    Unlike a reform, a level row needs no reform simulation: the microcosm value
    ...
    # ignores IRS credit-ordering, so the microcosm side remains a slight upper
    # bound.
...
def _state_program_levels_config_path() -> Path:
    return Path(str(files("microcosm.build.us").joinpath("state_program_levels.json")))
...
def state_program_level_specs(
    ...
    Each line compares the microcosm baseline total of a state credit variable
...
def _federal_eitc_by_state_config_path() -> Path:
    return Path(str(files("microcosm.build.us").joinpath("federal_eitc_by_state.json")))
...
def _state_spm_poverty_levels_config_path() -> Path:
    return Path(
        str(files("microcosm.build.us").joinpath("state_spm_poverty_levels.json"))
    )
...
def reform_validation_payload(
    ...
    *,
    ...
    microcosm budget effect is the calibrated ``final_estimate`` the calibration
    ...
    def released_weighted_total(
        ...
        microcosm#456: every out-of-sample score builds a full-release-sized
