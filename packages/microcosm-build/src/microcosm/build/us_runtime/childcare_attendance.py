...
def impute_us_childcare_attendance(
    ...
    *,
    ...
    pool = pool.sort_values("donor_id").reset_index(drop=True)
    groups = pool.groupby(list(match_columns), sort=False, dropna=False).indices
    ...
    for source_id, rows in result.groupby(
        ...
        if len(known) == len(US_CHILDCARE_ATTENDANCE_COLUMNS):
            ...
            key = tuple(replicas[column].iloc[0] for column in match_columns)
            # pandas uses scalar group keys for one matching column.
            group_key = key[0] if len(key) == 1 else key
            candidates = pool.iloc[groups.get(group_key, [])]
