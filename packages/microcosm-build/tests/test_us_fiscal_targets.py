...
def test_packaged_us_fiscal_resources_are_value_free() -> None:
    resource = files("microcosm.build.us").joinpath("fiscal_target_references.json")
...
def test_legacy_us_fiscal_value_manifest_is_not_packaged() -> None:
    resource = files("microcosm.build.us").joinpath("fiscal_targets.json")
...
def test_jct_obbba_no_tax_facts_do_not_compile_at_the_2024_target_period() -> None:
    ...
    # microcosm#451 items 3-4) ride the feed ahead of any binding: both
...
def test_ssa_ssi_recipients_national_fact_is_a_nonbinding_reference() -> None:
    # microcosm#508 owner adjudication (2026-07-23): the by-area national
...
def test_ssa_aged_category_row_can_never_bind_as_the_65_plus_age_band() -> None:
    """Role-separation guard (microcosm#508 adjudication, 2026-07-23).
...
def test_ssa_ssi_by_age_row_with_unknown_groupby_fails_closed() -> None:
    # microcosm#508 role separation, second layer: a row wearing the by-age
...
def test_stale_soi_taxable_interest_family_rebases_to_pub1304_national_control() -> (
    ...
    # microcosm#489 adjudication: HT2 is a correct TY2022 photo (Table 1.4
    ...
    # +128% frozen row of microcosm#489).
...
def test_stale_soi_taxable_interest_never_uses_congressional_district_controls() -> (
    ...
    # The microcosm#489 defect pin: the SOI congressional-district US
...
def test_stale_soi_taxable_interest_prefers_exact_universe_control_over_bridge() -> (
    ...
    # (Table 1.4 — the ledger follow-up to microcosm#489), it outranks the
...
def test_rebased_taxable_interest_rows_age_the_remaining_links_to_build() -> None:
    ...
    # quantity (microcosm#488 doctrine, microcosm#489 adjudication).
...
def test_cross_period_bounded_non_ht2_taxable_interest_slice_is_not_compiled() -> None:
    ...
    # rebase — a silent stale hard target, the microcosm#489 disease — so the
...
def test_rebased_capital_gains_rows_age_the_remaining_links_to_build() -> None:
    ...
    # family and the TY2023-sourced national row project the same 2024
    # quantity (microcosm#462 cross-vintage adjudication).
    from microcosm.build.us_runtime.target_aging import age_us_dollar_targets
    from microcosm.calibrate import TargetRegistry, TargetSpec
...
def test_uprated_row_with_unparseable_effective_period_fails_closed() -> None:
    # microcosm-authored uprating stamps must parse; a malformed one must
...
def test_soi_form_w2_social_security_tips_amount_targets_tip_income() -> None:
    # microcosm#451 item 3: the W-2 Box 7 dollar aggregate binds as a sum-mode
...
def test_ssa_ssi_age_band_counts_bind_as_person_age_indicator_targets() -> None:
    # microcosm#470: the SSA SSI Monthly age-band recipient counts bind as
...
def test_soi_table_2_1_mortgage_targets_bind_capped_deductible_concept() -> None:
    # microcosm#511: SOI Table 2.1 "Home mortgage interest" is the amount
...
def test_period_contract_raises_on_unaged_cross_period_dollars() -> None:
    # The microcosm#212 guard: an observation dollar level from TY2022 cannot
...
def test_period_contract_waiver_annotates_instead_of_raising() -> None:
    from microcosm.build.us_runtime.target_aging import enforce_period_contract
    from microcosm.calibrate import TargetRegistry, TargetSpec
...
def test_compile_enforces_the_period_contract_without_a_waiver() -> None:
    # Compile-level wiring for the microcosm#212 guard: the same surface the
    # waived tests compile above must refuse to build silently un-aged.
    import pytest

    from microcosm.build.us_runtime.target_aging import PeriodContractError
