...
def _uc_contract(target: dict) -> dict:
    ...
    target.setdefault("label", "Universal Credit households")
    return {
        "schema_version": 2,
        "country": "uk",
    ...
    for index, month in enumerate(months):
        for cell, operand in enumerate(operands):
            ...
            fact["dimension_labels"] = {
                ...
                "payment_indicator": "Payment status",
                "child_entitlement": "Child entitlement",
            }
            fact["dimension_value_labels"] = {
                dimension_id: {str(value): str(value).replace("_", " ").capitalize()}
                for dimension_id, value in fact["dimensions"].items()
            }
    ...
    assert resource == {
        ...
        "hierarchy": {
            **UC_HIERARCHY,
            ...
            "target_labels": {"uc.calendar": "Universal Credit households"},
        },
...
def _fact(month: str, *, suffix: str = "", value: float = 100.0) -> dict:
    return {
        ...
        "label": f"Universal Credit households, {month}",
